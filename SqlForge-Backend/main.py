...
from models import ConnectionConfig, QueryRequest, QueryResult, TableInfo, AIRequest, SyncRequest, TableSchema, AlterTableRequest, FederatedQueryRequest, FederatedQueryResult
import database
import internal_db
...
@app.post("/pro/backup")
def run_backup(request: Dict[str, Any]):
    # request: { connection_id, incremental, native }
    conn_id = request.get("connection_id")
    incremental = request.get("incremental", False)
    native = request.get("native", True)
    
    config = internal_db.get_connection(conn_id)
    if not config:
        raise HTTPException(status_code=404, detail="Connection not found")
        
    return backup.backup_database(config, incremental=incremental, native=native)

@app.post("/pro/restore")
def run_restore(request: Dict[str, Any]):
    # request: { connection_id, backup_path }
    conn_id = request.get("connection_id")
    ...
    return backup.restore_database(config, backup_path)

@app.post("/pro/execute-script")
def execute_db_script(request: Dict[str, Any]):
    # request: { connection_id, file_path }
    conn_id = request.get("connection_id")
    ...
    if config.type == 'redis':
        return backup.execute_redis_commands(config, file_path)
    else:
        return backup.execute_sql_file(config, file_path)
...
@app.get("/workspaces/{conn_id}")
def list_workspaces(conn_id: str):
    return internal_db.get_model_workspaces(conn_id)

@app.get("/workspaces/load/{workspace_id}")
def load_workspace(workspace_id: str):
    ws = internal_db.get_model_workspace(workspace_id)
    if not ws:
        raise HTTPException(status_code=404, detail="Workspace not found")
    return ws

@app.post("/workspaces/save")
def save_workspace(request: Dict[str, Any]):
    # request: { id, connection_id, name, content }
    ws_id = request.get("id") or str(uuid.uuid4())
    internal_db.save_model_workspace(
        ws_id,
...
def delete_workspace(workspace_id: str):
    internal_db.delete_model_workspace(workspace_id)
    return {"status": "deleted"}
...
@app.get("/favorites")
def list_favorites():
    return internal_db.get_favorites()

@app.post("/favorites")
def add_favorite(request: Dict[str, Any]):
    # request: { type, name, connection_id, target }
    fav_id = str(uuid.uuid4())
    internal_db.save_favorite(
        fav_id,
        request["type"],
        request["name"],
        request["connection_id"],
...
def remove_favorite(fav_id: str):
    internal_db.delete_favorite(fav_id)
    return {"status": "deleted"}
...
if __name__ == "__main__":
    ...
    if is_frozen:
        # If packaged, don't use string format and reload
        uvicorn.run(app, host="127.0.0.1", port=8000)
    else:
        # In development, reload is useful
