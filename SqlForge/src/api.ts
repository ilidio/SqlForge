...
export const api = {
  ...
  runQuery: (connId: string, sql: string, queryId?: string, timeoutSeconds?: number) => axios.post<{columns: string[], rows: Record<string, unknown>[], error: string | null}>(`${API_URL}/query`, { connection_id: connId, sql, query_id: queryId, timeout_seconds: timeoutSeconds }).then(r => r.data),
