...
import { Download, FileSpreadsheet, FileJson, FileCode, CheckCircle2, ChevronRight, Settings2, Database, ShieldCheck, FileText } from 'lucide-react';
...
export default function ExportWizard({ open, onOpenChange, connectionId, tableName }: ExportWizardProps) {
    ...
    const [format, setFormat] = useState<'csv' | 'json' | 'sql' | 'excel' | 'xml' | 'txt' | 'dbf'>('csv');
    ...
    const formats = [
        { id: 'csv', name: 'CSV (Comma Separated)', icon: FileSpreadsheet, color: 'text-emerald-500', desc: 'Best for Excel & Analysis' },
        { id: 'excel', name: 'MS Excel (.xlsx)', icon: FileSpreadsheet, color: 'text-green-600', desc: 'Native Excel format' },
        { id: 'json', name: 'JSON (Object Notation)', icon: FileJson, color: 'text-amber-500', desc: 'Web APIs & App Integration' },
        { id: 'xml', name: 'XML (Markup Language)', icon: FileCode, color: 'text-orange-500', desc: 'Legacy systems & Data exchange' },
        { id: 'txt', name: 'Plain Text (.txt)', icon: FileText, color: 'text-slate-500', desc: 'Simple text dump' },
        { id: 'dbf', name: 'dBase (DBF)', icon: Database, color: 'text-blue-600', desc: 'Legacy databases & GIS' },
        { id: 'sql', name: 'SQL (INSERT statements)', icon: FileCode, color: 'text-blue-500', desc: 'Database Migrations' },
    ];
