...
import { Database, Table, Plus, RefreshCw, History, Clock, ChevronRight, ChevronDown, Layers, FileText, Key, Box, Search, Settings, Zap, Cpu, Terminal, Wand2, Activity, Sparkles, Play, Star } from 'lucide-react';
  ...
  const { theme, setTheme } = useTheme();
    ...
    const loadFavorites = React.useCallback(async () => {
      ...
      useEffect(() => {
        ...
        if (activeTab === 'connections') loadFavorites();
  ...
  return (
    <div className="w-64 bg-sidebar border-r border-sidebar-border flex flex-col h-full select-none text-sm shadow-sm z-20">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
        ...
        <TabsContent value="connections" className="flex-1 mt-0 outline-none overflow-hidden flex flex-col">
            {favorites.length > 0 && (
                <div className="mb-4">
                    ...
                    <div className="px-3 space-y-0.5">
                        {favorites.map(fav => (
                            <div 
                                key={fav.id} 
                                className="flex items-center gap-2 px-2 py-1 hover:bg-sidebar-accent/50 rounded-md cursor-pointer text-xs group"
                                onClick={() => {
                                    if (fav.type === 'table') onSelectTable(fav.connection_id, fav.target);
                                    if (fav.type === 'query') onOpenQuery(fav.connection_id, fav.target);
                                }}
                            >
                                {fav.type === 'table' ? <Table size={12} className="text-blue-500" /> : <Terminal size={12} className="text-purple-500" />}
                                ...
                                <Button 
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        api.removeFavorite(fav.id).then(() => loadFavorites());
                                    }}
                                >
                                    <X size={10} />
                                </Button>
                                                                ...
                                                                {typeItems.map(t => (
                                                                    <ContextMenu key={t.name}>
                                                                        ...
                                                                        <ContextMenuContent className="w-56">
                                                                            ...
                                                                            <ContextMenuItem 
                                                                                onClick={() => api.addFavorite('table', t.name, conn.id!, t.name).then(() => loadFavorites())}
                                                                                className="gap-2"
                                                                            >
                                                                                <Star size={14} className="text-amber-500" /> Add to Favorites
                                                                            </ContextMenuItem>
