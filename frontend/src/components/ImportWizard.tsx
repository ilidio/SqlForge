...
import { Upload, FileSpreadsheet, FileJson, CheckCircle2, ChevronRight, Settings2, FileText, X, Database, Code } from 'lucide-react';
...
export default function ImportWizard({ open, onOpenChange, connectionId, tableName }: ImportWizardProps) {
    ...
    const [format, setFormat] = useState<'csv' | 'json' | 'xml' | 'excel' | 'dbf' | 'txt'>('csv');
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                    {[
                                        { id: 'csv', name: 'CSV', icon: FileSpreadsheet, color: 'text-emerald-500' },
                                        { id: 'excel', name: 'Excel', icon: FileSpreadsheet, color: 'text-green-600' },
                                        { id: 'json', name: 'JSON', icon: FileJson, color: 'text-amber-500' },
                                        { id: 'xml', name: 'XML', icon: Code, color: 'text-orange-500' },
                                        { id: 'txt', name: 'Text', icon: FileText, color: 'text-slate-500' },
                                        { id: 'dbf', name: 'DBF', icon: Database, color: 'text-blue-600' },
                                    ].map((f) => (
                                        <div 
                                            key={f.id}
                                            className={cn(
                                                "p-3 rounded-xl border cursor-pointer transition-all flex items-center gap-3",
                                                format === f.id ? "border-primary bg-primary/5 ring-1 ring-primary shadow-sm" : "hover:bg-muted/10"
                                            )}
